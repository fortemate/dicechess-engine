...
- `shared-rules/src/main/scala/dicechess/engine/` — the **rules core**, published separately as `com.fortemate:dicechess-rules_3` (ADR 009 / #218). No third-party dependencies; `rules-smoke/` compiles against this jar alone. Cross-compiled (JVM + JS + Wasm):
  - `domain/` — opaque-type game state: `Bitboard`/`Square`/`Piece`/`Color` (`Models.scala`), `Position`, `GameFlags`, `Move`, `FenParser` (DFEN), `Symmetry`, `Zobrist`.
  - `movegen/` — `MagicBitboards`, `LeaperAttacks`, `PawnGeneration`, `MoveGenerator`, `LegalMovesFilter`, `Dfen`, `Perft`. Allocation-sensitive hot path.
  - `search/` (rules part) — `TurnGenerator` (exhaustive legal micro-move paths), `KingCaptureProbability` (+ `KcpScratchBoard`, 216 dice outcomes), `DiceRolls`. They deliberately keep the package `dicechess.engine.search`: the package is split across the two jars and no consumer changes an import.
- `shared/src/main/scala/dicechess/engine/` — the rest of the cross-compiled core, published as `dicechess-engine_3` and depending on the rules artifact:
...
- `jvm/src/main/scala/dicechess/engine/` — entry point `Main.scala` (JLine REPL CLI, `cli/`), JVM-only ONNX inference bots (`search/OnnxEvalSearch.scala`, `OnnxExpectimaxSearch.scala` on onnxruntime), `model/` — the ONNX serving contract (`ModelManifest`, `ModelRole`, `FeatureSchema`, `OnnxModelContract`, `ModelPackage`: everything checked before a trained artifact is wired into a search, see the docs page *Model Serving Contract*), `json/Json.scala` — the repository's one hand-rolled JSON codec (`private[engine]`, read by the manifest loader and the arena runners' `--json` reports), and `jvmapi/JvmApi.scala` — the Java/Kotlin-facing facade (the JVM row's counterpart to `js/`'s `EngineFacade`). ONNX bots are absent from the npm bundles.
...
- JSON fixtures that remain are JVM-only: `shared-rules/src/test/resources/movegen/perft_suite.json` (`PerftSpec`). King-capture probability cases live in the shared Scala fixture `shared-rules/src/test/scala/dicechess/engine/search/KingCaptureFixtures.scala`, which also feeds `KingCaptureDocGenerator`.
...
- Docs site: `docs/` (Astro + Starlight, mermaid + KaTeX), deployed together with Scaladoc to GitHub Pages by `deploy-docs.yaml` on pushes to `main` touching `docs/**`, `{shared-rules,shared,jvm,js}/src/main/scala/**`, the movegen fixture sources, the KCP fixture source, or the workflow itself. Local dev: `mise run docs:dev`.
- Update-trigger map:
