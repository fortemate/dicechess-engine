...
- Never commit to `main`. Branch: `<type>/<short-desc>` or `<type>/<id>-<short-desc>`
  ...
  of `feat`). Carry an issue id only when the PR will fully complete that issue, and include
  `Closes #<id>` in the PR body. For partial work, use a sub-issue id or a non-closing reference.
