...
Since capturing the opponent's king is the ultimate win condition, **any sequence of micro-moves that ends with capturing the opponent's king (whether 1, 2, or 3 micro-moves long) is always legal**, even if it terminates the turn early and does not achieve the maximum possible micro-move count. Capturing the king wins the game on the spot.
...
1. There exists a playable continuation sequence $(m_1, m_2, \dots, m_k)$ ($k \in \{1, 2, 3\}$) that captures the opponent's King (i.e. $\text{isKingCapture}(S_{k-1}, m_k) = \text{true}$), **OR**
...
If a sequence of moves $(m_1, \dots, m_k)$ captures the opponent's king, the entire sequence is legally valid and terminates the game immediately upon the king capture. The first move $m_1$ is fully legal even if $k < L^*(S_0, D)$. We do not evaluate subsequent moves for the remaining dice once the King is captured.
...
graph TD
    ...
    B1 -- No: e7-e8=Q --> F1[No Knight on board] --> G1[Cannot play Knight move] --> H1[Sequence length = 2 - ILLEGAL]
    end
